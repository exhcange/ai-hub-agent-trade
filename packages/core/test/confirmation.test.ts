...
test("confirmation is bound to its original context and is one-time", () => {
  ...
  const confirmed = confirmations.confirm(prepared.confirmationId, true, context);
  assert.equal(confirmed.action, "spot_place_order");
  assert.equal(confirmed.payload.symbol, "btcusdt");
  assert.throws(() => confirmations.confirm(prepared.confirmationId, true, context), { code: "AI_HUB_CONFIRMATION_NOT_FOUND" });
});
...
test("confirmation is invalid when a credential version changes", () => {
  ...
  assert.throws(
    () => confirmations.confirm(prepared.confirmationId, true, { ...context, credentialVersion: "credential-v2" }),

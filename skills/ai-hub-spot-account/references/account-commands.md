...
`account transfer` supports only `EXCHANGE` (Spot) and `FUTURE` (Derivatives). Use `EXCHANGE -> FUTURE` for Spot to Derivatives and the reverse for Derivatives to Spot. Do not pass numeric account types such as `1` or `5`; use `wallet transfer` for numeric account-type transfers. The transfer command is state-changing. It prints a preview and requires the user to enter `yes` manually in an interactive terminal. Do not pipe or inject the confirmation.
